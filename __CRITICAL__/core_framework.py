...
class CRM:
    # global self.args, self.potentiation, self.constantPot, self.positionalActivityBias, self.sess,
    # self.train_step, self.myLoss, self.mseTF, self.mergedSummaries, self.global_step,
    # self.ohcX, self.realELY, self.summaryWriter,self.epBoundTensor, self.epBoundTensorRC,
    # self.seqPotentialTensor, self.motifsTensor2, self.logConcs, self.activities, self.constant,
    # self.saver, self.saveParams, self.positionalActivityBiasRC, self.activityDiffs, BASES
    ...
    def makeModel(self):
        ...
        if os.path.isfile(self.args.outFPre + ".done"):
            if self.args.verbose > 1: sys.stderr.write(
                "Deleting old \"done\" file %s\n" % (self.args.outFPre + ".done"));
        ...
        while runsLeft > 0:
            ...
            if self.args.verbose > 3: sys.stderr.write(
                "  Time for last non-TF code: %f\n" % (datetime.now() - curTimeNonTF).total_seconds())
            curTimeTF = datetime.now()
            if self.args.trace > 0 and globalStep > 100:  # don't take the first one
                run_metadata = tf.RunMetadata()
                _, curLoss, curMSE, summaries, globalStep = self.sess.run(
                    [self.train_step, self.myLoss, self.mseTF, self.mergedSummaries, self.global_step],
                    feed_dict={self.ohcX: batchX, self.realELY: batchY},
                ...
                quit();
            elif self.args.tensorboard is not None and (
                    (self.args.tensorboardFrequency is None and nextTBF == (globalStep + 1)) or (
                    self.args.tensorboardFrequency is not None and (
                    globalStep + 1) % self.args.tensorboardFrequency == 0)):
                ...
                _, curLoss, curMSE, globalStep = self.sess.run(
                    [self.train_step, self.myLoss, self.mseTF, self.global_step],
                    feed_dict={self.ohcX: batchX, self.realELY: batchY})
            if self.args.verbose > 3: sys.stderr.write(
                "  Time for last TF code: %f\n" % (datetime.now() - curTimeTF).total_seconds())
            curTimeNonTF = datetime.now()
            runningMeanMSE[globalStep % self.args.runningAverageWindow] = curMSE;
            runningMSE = np.nanmean(runningMeanMSE);
            if self.args.verbose > 3 or (self.args.verbose > 2 and (globalStep % 100) == 0) or (
                    self.args.verbose > 1 and (globalStep % 10000) == 0):
                curTime = datetime.now()
                deltaTime = (curTime - lastTime).total_seconds()
                lastTime = curTime;
                if self.args.L1 is not None:
                    sys.stderr.write(
                        "	Batch = %i; examples = %i;\trunning MSE = %.3f;\tlast MSE = %.2f;\tlast time = %.2f\tloss = %.2f;\t paramsum = %.2f;\tparamnum = %i\n" % (
                        globalStep, globalStep * self.args.batch, runningMSE, curMSE, deltaTime, curLoss,
                        paramPenaltyL1Tensor.eval(session=self.sess), paramNumActivityTensor.eval(session=self.sess)));
                else:
                    sys.stderr.write(
                        "	Batch = %i; examples = %i;\trunning MSE = %.3f;\tlast MSE = %.2f\tlast time = %.2f\n" % (
                        globalStep, globalStep * self.args.batch, runningMSE, curMSE, deltaTime));
            # save state, if desired
            if self.args.tensorboard is not None and (
                    (self.args.tensorboardFrequency is None and nextTBF == globalStep) or (
                    self.args.tensorboardFrequency is not None and globalStep % self.args.tensorboardFrequency == 0)):
        ...
        if self.args.tensorboard is not None:
            _, curLoss, curMSE, summaries = self.sess.run(
                [self.train_step, self.myLoss, self.mseTF, self.mergedSummaries],
                feed_dict={self.ohcX: batchX, self.realELY: batchY})
        ...
        builder.add_meta_graph_and_variables(self.sess, [
            "main"])  # , signature_def_map=foo_signatures, assets_collection=foo_assets)
    ...
    def makeGraph(self):
        ...
        # sequence layer
        self.ohcX = tf.placeholder(tf.float32, [None, 4, self.args.seqLen, 1],
                                   name="ohcX")  # here, [None, 4,...] indicate that the first dimention is of unknown length
        # motif parameters:
        # allow for the user to provide a list of PWMs that we initialize the motifs tensor to
        if self.args.motifsFP is not None:
            if self.args.verbose > 0: sys.stderr.write(
                "Reading in default set of TF Motifs from %s\n" % (self.args.motifsFP));
            # motifsFP is a one-per-line list of PWM paths
            motifsFile = MYUTILS.smartGZOpen(self.args.motifsFP, 'r');
            defaultMotifs = np.random.normal(0, 1, [4, self.args.motifLen, self.args.numMotifs])  # Gaussian(0,1)
            i = 0
            for line in motifsFile:
                ...
                if pwmLen > self.args.motifLen:  # can't fit the motif;
                    # if cannot feed into the entire motif, just feed in the beginning part
                ...
                for bi in range(0, len(BASES)):
                    ...
                    defaultMotifs[bi, paddingFront:(paddingFront + pwmEnd - pwmStart), i] = curPWM.mat[b][
                                                                                            pwmStart:pwmEnd];
                i += 1;
            ...
            if self.args.noTrainMotifs > 0:
                motifsTensor = tf.constant(
                    np.random.standard_normal([4, self.args.motifLen, self.args.numMotifs]).astype(np.float32),
                    name="motifs")
            else:
                motifsTensor = tf.Variable(tf.random_normal([4, self.args.motifLen, self.args.numMotifs]),
                                           name="motifs")

        motifsRCTensor = tf.image.flip_left_right(tf.image.flip_up_down(
            motifsTensor))  # flips along first two dims; this works because order of bases is ACGT=rc(TGCA)

        self.motifsTensor2 = tf.reshape(motifsTensor, [4, self.args.motifLen, 1,
                                                       self.args.numMotifs]);  # patch height, patch width, input channels, output channels
        ...
        if self.args.noTrainActivities == 0:
            ...
            self.activities = tf.Variable(tf.convert_to_tensor(initActiv.reshape([self.args.numMotifs])),
                                          name="activities");
        else:
        ...
        if self.args.trainPositionalActivities > 0:
            ...
            self.positionalActivityBias = tf.Variable(tf.ones([self.args.seqLen, self.args.numMotifs]),
                                                      name="positionalActivities")  # [seqLen, numMotifs]
            ...
            if self.args.trainStrandedActivities:
                self.positionalActivityBiasRC = tf.Variable(tf.ones([self.args.seqLen, self.args.numMotifs]),
                                                            name="positionalActivitiesRC")  # [seqLen, numMotifs]
        ...
        if self.args.noTrainConcs == 0:
            ...
            if not self.args.VARIABLE:
                self.logConcs = tf.Variable(tf.convert_to_tensor(initConcs.reshape((1, 1, 1, self.args.numMotifs))),
                                            name="concentrations");
            else:
                self.logConcs = tf.Variable(tf.convert_to_tensor(initConcs.reshape((1, 1, 1, self.args.numMotifs))),
                                            name="Variable");
            ...
            self.logConcs = tf.constant(initConcs.reshape((1, 1, 1, self.args.numMotifs)),
                                        name="concentrations");  ###TODO name="concentrations"
        ...
        if self.args.bindingLimits:
            ...
            if self.args.initBindLim is not None:
                if self.args.verbose > 0: sys.stderr.write(
                    "Initializing binding limits to %s\n" % self.args.initBindLim)
            ...
            if self.args.noTrainBL == 0:
                ...
                logBindingLimits = tf.Variable(tf.convert_to_tensor(initBL.reshape((self.args.numMotifs))),
                                               name="logBindingLimits");  # [numMotifs]
            else:
                if self.args.verbose > 0: sys.stderr.write("Not training binding limits\n")
                logBindingLimits = tf.constant(initBL.reshape((self.args.numMotifs)),
                                               name="logBindingLimits");  # [numMotifs]
        ...
        # these are log(Kds)
        motifScanTensor = tf.nn.conv2d(self.ohcX, self.motifsTensor2, strides=[1, 4, 1, 1], padding='SAME',
                                       name="motifScan")
        motifScanRCTensor = tf.nn.conv2d(self.ohcX, motifsRCTensor2, strides=[1, 4, 1, 1], padding='SAME',
                                         name="motifScanRC")
        ...
        if self.args.useEBound > 0:
            ...
            self.epBoundTensor = tf.add(tf.reduce_sum(tf.subtract(1.0, pNotBoundRCTensor), reduction_indices=[1, 2]),
                                        tf.reduce_sum(tf.subtract(1.0, pNotBoundTensor), reduction_indices=[1,
                                                                                                            2]))  # size: [None, numMotifs] #expected amount of binding
        else:
            if self.args.verbose > 0: sys.stderr.write("Using P-bound\n")
            self.epBoundTensor = tf.subtract(1.0,
                                             tf.multiply(tf.reduce_prod(pNotBoundRCTensor, reduction_indices=[1, 2]),
                                                         tf.reduce_prod(pNotBoundTensor, reduction_indices=[1,
                                                                                                            2])))  # size: [None, numMotifs] # p(bound)
        ...
        if self.args.potentiation > 0:
            ...
            if self.args.noTrainPotentiations == 0:
                ...
                self.potentiation = tf.Variable(tf.convert_to_tensor(initPotent.reshape([self.args.numMotifs])),
                                                name="potents");
            else:
            ...
            self.seqPotentialTensor = tf.sigmoid(
                tf.add(tf.reduce_sum(seqPotentialByTFTensor, reduction_indices=[1]), self.constantPot),
                name="seqPotentialTensor")  # [None, 1]
        else:
        ...
        if self.args.trainPositionalActivities > 0:  # account for positional activity with linear scaling of activity
            if self.args.trainStrandedActivities > 0:  # account for strand-specific activity biases
                ...
                if self.args.potentiation > 0:
                    pBoundPerPos = tf.transpose(
                        tf.multiply(tf.transpose(pBoundPerPos, perm=(1, 2, 3, 0)), self.seqPotentialTensor),
                        perm=(3, 0, 1, 2))  # size: None,1,seqLen,numMotifs]
                    pBoundPerPosRC = tf.transpose(
                        tf.multiply(tf.transpose(pBoundPerPosRC, perm=(1, 2, 3, 0)), self.seqPotentialTensor),
                        perm=(3, 0, 1, 2))  # size: None,1,seqLen,numMotifs]
                # print(tf.Tensor.get_shape(pBoundPerPos))
                # print(tf.Tensor.get_shape(positionalActivity))
                if self.args.bindingLimits > 0:
                    expectedActivitySense = tf.reduce_sum(tf.multiply(pBoundPerPos, positionalActivity),
                                                          reduction_indices=[1, 2])  # size: [None,numMotifs]
                    expectedActivityRC = tf.reduce_sum(tf.multiply(pBoundPerPosRC, positionalActivityRC),
                                                       reduction_indices=[1, 2])  # size: [None,numMotifs]
                    expectedActivityPerTF = tf.add(
                        # min of positive self.activities and max of negative self.activities accounting for binding limits.
                        tf.nn.relu(tf.minimum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            tf.add(expectedActivitySense, expectedActivityRC))),  # positive
                        tf.negative(tf.nn.relu(tf.negative(tf.maximum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            tf.add(expectedActivitySense, expectedActivityRC)))))  # negative
                    )  # [None,numMotifs]
                ...
                if self.args.useEBound > 0:
                    pBoundPerPos = tf.add(tf.subtract(1.0, pNotBoundTensor),
                                          tf.subtract(1.0, pNotBoundRCTensor))  # size: [None,1,seqLen,numMotifs]
                else:
                    pBoundPerPos = tf.subtract(1.0, tf.multiply(pNotBoundTensor,
                                                                pNotBoundRCTensor))  # size: [None,1,seqLen,numMotifs]
                # print(tf.Tensor.get_shape(pBoundPerPos))
                if self.args.potentiation > 0:
                    pBoundPerPos = tf.transpose(
                        tf.multiply(tf.transpose(pBoundPerPos, perm=(1, 2, 3, 0)), self.seqPotentialTensor),
                ...
                if self.args.bindingLimits > 0:
                    ...
                    expectedActivityPerTF = tf.add(
                        ...
                        tf.nn.relu(tf.minimum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            expectedActivitySense)),  # positive
                        tf.negative(tf.nn.relu(tf.negative(tf.maximum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            expectedActivitySense))))  # negative
                    )  # [None,numMotifs]
                else:
                    expectedActivityPerTF = tf.multiply(
                        tf.reshape(pBoundPerPos, (-1, self.args.seqLen * self.args.numMotifs)),
                        tf.reshape(positionalActivity,
                                   (self.args.seqLen * self.args.numMotifs, 1)))  # size: [None,numMotifs]
        else:  # no positional self.activities
            if self.args.trainStrandedActivities > 0:  # account for strand-specific activity biases
                if self.args.useEBound > 0:
                    self.epBoundTensor = tf.reduce_sum(tf.subtract(1.0, pNotBoundTensor), reduction_indices=[1, 2],
                                                       name="epBoundTensor")  # size: [None, numMotifs] #expected amount of binding
                    self.epBoundTensorRC = tf.reduce_sum(tf.subtract(1.0, pNotBoundRCTensor), reduction_indices=[1, 2],
                                                         name="epBoundTensorRC")  # size: [None, numMotifs] #expected amount of binding
                else:
                    self.epBoundTensor = tf.subtract(1.0, tf.reduce_prod(pNotBoundTensor, reduction_indices=[1, 2]),
                                                     name="epBoundTensor")  # numMotifs: [None, numMotifs] # p(bound)
                    self.epBoundTensorRC = tf.subtract(1.0, tf.reduce_prod(pNotBoundRCTensor, reduction_indices=[1, 2]),
                                                       name="epBoundTensorRC")  # size: [None, numMotifs] # p(bound)
                if self.args.potentiation > 0:
                    self.epBoundTensor = tf.transpose(
                        tf.multiply(tf.transpose(self.epBoundTensor), self.seqPotentialTensor),
                        name="epBoundTensor");  # [None, numMotifs]
                    self.epBoundTensorRC = tf.transpose(
                        tf.multiply(tf.transpose(self.epBoundTensorRC), self.seqPotentialTensor),
                        name="epBoundTensorRC");  # [None, numMotifs]
                ...
                if self.args.bindingLimits > 0:  # note that when adding strand-specific self.activities when there are binding limits, the output will change without changing the params because you can't limit both simultaneously now.
                    expectedActivityPerTF = tf.add(
                        ...
                        tf.nn.relu(tf.minimum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            tf.add(tf.multiply(self.epBoundTensor, self.activities),
                                   tf.multiply(self.epBoundTensorRC, activitiesRC)))),  # positive
                        tf.negative(tf.nn.relu(tf.negative(tf.maximum(
                            tf.reshape(tf.multiply(bindingLimits, self.activities), (1, self.args.numMotifs)),
                            tf.add(tf.multiply(self.epBoundTensor, self.activities),
                                   tf.multiply(self.epBoundTensorRC, activitiesRC))))))  # negative
                    )  # [None,numMotifs]
                # sys.stderr.write(" ".join([str(x) for x in tf.Tensor.get_shape(expectedActivity)])+"\n")
                else:
                    expectedActivityPerTF = tf.add(
                        tf.multiply(self.epBoundTensor, tf.reshape(self.activities, (self.args.numMotifs, 1))),
                        tf.multiply(self.epBoundTensorRC,
                                    tf.reshape(activitiesRC, (self.args.numMotifs, 1))))  # [None,numMotifs]
                ...
                if self.args.potentiation > 0:
                    self.epBoundTensor = tf.transpose(
                        ...
                        name="epBoundTensor");  # [None,numMotifs]
                if self.args.bindingLimits > 0:
                    self.epBoundTensor = tf.minimum(self.epBoundTensor, bindingLimits,
                                                    name="epBoundTensor");  # [None, numMotifs]
        ...
        if self.args.accIsAct > 0:
            ...
            predELY = tf.add(tf.add(tf.reshape(expectedActivity, [-1]), tf.reshape(accActivELTensor, [-1])),
                             self.constant, name="predELY")  # size: [None]
        else:
        ...
        if self.args.meanSDFile is not None:
            ...
            if self.args.verbose > 0: sys.stderr.write(
                "Minimizing the negative log liklihood with an SD vector of length %i and a maximum value of %f\n" % (
                llSDLen, maxllMean))
            llSDTensor = tf.constant(llSDs, name="llSDs")
            predELLLIndeces = tf.minimum(llSDLen - 1, tf.maximum(0, tf.to_int32(
                tf.round(tf.multiply(predELY, (llSDLen / maxllMean))))), name="predELLLInd")
        ...
        if self.args.L2 is not None and self.args.noTrainMotifs == 0:
            if self.args.verbose > 0: sys.stderr.write(
                "Using L2 regularization of PWMs with lambda=%s\n" % (self.args.L2))
            self.args.L2 = float(self.args.L2);
            paramPenaltyL2Tensor = tf.nn.l2_loss(
                motifsTensor)  # doesn't make sense to l2 loss the concentrations since this brings them to 0; however, bringing the PWM entries to 0 does make sense.
            self.myLoss = tf.add(self.myLoss, tf.multiply(paramPenaltyL2Tensor, self.args.L2));

        if self.args.L1 is not None:
            if self.args.verbose > 0: sys.stderr.write(
                "Using L1 regularization of self.activities with lambda=%s\n" % (self.args.L1))
            ...
            if self.args.potentiation > 0:
                ...
                paramNumActivityTensor = paramNumActivityTensor + tf.reduce_sum(
                    tf.cast(tf.greater(tf.abs(self.potentiation), EPSILON), tf.int32))
            if self.args.trainStrandedActivities > 0:
                paramPenaltyL1Tensor = paramPenaltyL1Tensor + tf.reduce_sum(tf.abs(self.activityDiffs))
                paramNumActivityTensor = paramNumActivityTensor + tf.reduce_sum(
                    tf.cast(tf.greater(tf.abs(self.activityDiffs), EPSILON), tf.int32))
            if self.args.accIsAct > 0:
                paramPenaltyL1Tensor = paramPenaltyL1Tensor + tf.abs(accActivity)
                paramNumActivityTensor = paramNumActivityTensor + tf.reduce_sum(
                    tf.cast(tf.greater(tf.abs(accActivity), EPSILON), tf.int32))
            if self.args.trainPositionalActivities > 0:
                paramPenaltyL1Tensor = paramPenaltyL1Tensor + tf.reduce_sum(tf.abs(
                    tf.slice(self.positionalActivityBias, [0, 0],
                             [1, self.args.numMotifs])))  # penalize only the first column of positional self.activities
                # So only one position per TF is L1; the differences between the others are L2
                paramNumActivityTensor = paramNumActivityTensor + tf.reduce_sum(tf.cast(
                    tf.greater(tf.abs(tf.slice(self.positionalActivityBias, [0, 0], [1, self.args.numMotifs])),
                               EPSILON), tf.int32))
                if self.args.trainStrandedActivities > 0:
                    paramPenaltyL1Tensor = paramPenaltyL1Tensor + tf.reduce_sum(
                        tf.abs(tf.slice(self.positionalActivityBiasRC, [0, 0], [1, self.args.numMotifs])))
                    paramNumActivityTensor = paramNumActivityTensor + tf.reduce_sum(tf.cast(
                        tf.greater(tf.abs(tf.slice(self.positionalActivityBiasRC, [0, 0], [1, self.args.numMotifs])),
                                   EPSILON), tf.int32))
        ...
        if self.args.L2Pos is not None and self.args.trainPositionalActivities > 0:
            ...
            paramPenaltyL2PosTensor = tf.reduce_sum(tf.abs(
                tf.subtract(tf.slice(self.positionalActivityBias, [0, 0], [self.args.seqLen - 1, self.args.numMotifs]),
                            tf.slice(self.positionalActivityBias, [1, 0],
                                     [self.args.seqLen - 1, self.args.numMotifs]))))
            if self.args.trainStrandedActivities > 0:
                paramPenaltyL2PosTensor = paramPenaltyL2PosTensor + tf.nn.l2_loss(tf.subtract(
                    tf.slice(self.positionalActivityBiasRC, [0, 0], [self.args.seqLen - 1, self.args.numMotifs]),
                    tf.slice(self.positionalActivityBiasRC, [1, 0], [self.args.seqLen - 1, self.args.numMotifs])))
        ...
        if self.args.cycleLearningRate is not None:
            if self.args.verbose > 0: sys.stderr.write(
                "Cycling learning rate low,high,period %s\n" % self.args.cycleLearningRate)
            clrParams = self.args.cycleLearningRate.split(",")
            learningRate = TFHELP.cycle_learning_rate(float(clrParams[0]), float(clrParams[1]), self.global_step,
                                                      int(clrParams[2]), name="learningRate")
        else:
            learningRate = tf.train.exponential_decay(learningRate, self.global_step, 175, self.args.learningRateED,
                                                      staircase=True, name="learningRate")
        if self.args.useMomentumOptimizer > 0 or self.args.useRMSProp > 0:
            ...
            if self.args.useMomentumOptimizer > 0:
                ...
                opt = tf.train.MomentumOptimizer(self.args.learningRate, momentum,
                                                 use_nesterov=self.args.useNesterov > 0);

            elif self.args.useRMSProp > 0:
                if self.args.verbose > 0: sys.stderr.write("Using RMSProp instead of Adam\n")
                opt = tf.train.RMSPropOptimizer(self.args.learningRate, decay=self.args.rmsDecay, momentum=momentum,
                                                epsilon=self.args.rmsEpsilon);
        else:
            if self.args.verbose > 0: sys.stderr.write("Using AdamOptimizer\n")
            opt = tf.train.AdamOptimizer(self.args.learningRate, epsilon=self.args.Aepsilon, beta1=self.args.Abeta1,
                                         beta2=self.args.Abeta2);
        ...
        if self.args.trace > 2:
            self.sess = tf.Session(
                config=tf.ConfigProto(intra_op_parallelism_threads=self.args.threads - 1, log_device_placement=True));
        else:
        ...
        if self.args.restore is not None:
            ...
            if "concentrations" in reader.get_variable_to_shape_map().keys() and self.args.VARIABLE:
                raise Exception(
                    "BACKWARD COMPATIBILITY ERROR: loaded graph has variable named concentrations - re-run withOUT -VARIABLE option\n");
            ...
            restoringThese = {k: restoringThese[k] for k in list(restoringThese.keys()) if
                              k in reader.get_variable_to_shape_map().keys()}
            import re;
            restoringThese = {k: restoringThese[k] for k in list(restoringThese.keys()) if
                              re.search("global_step", k) is None}
            if self.args.clearAdamVars > 0:
                restoringThese = {k: restoringThese[k] for k in list(restoringThese.keys()) if
                                  re.search("/Adam", k) is None}
                restoringThese = {k: restoringThese[k] for k in list(restoringThese.keys()) if
                                  re.search("beta[12]_power", k) is None}
            if self.args.verbose > 1:
                sys.stderr.write(
                    "Loading these variables: %s\n" % (", ".join([k for k in list(restoringThese.keys())])))
        ...
        if len([k.name for k in tf.trainable_variables() if
                re.search("Variable", k.name) is not None]) > 0 and not self.args.VARIABLE:
            print([k.name for k in tf.trainable_variables()])
            raise (Exception("Error: one or more variables with default names: %s" % ", ".join(
                [k.name for k in tf.trainable_variables() if re.search("Variable", k.name) is not None])));
    ...
    def saveParams(self, sess):
        ...
        if self.args.trainPositionalActivities > 0:
            ...
            positionalActivityBiasVals = self.positionalActivityBias.eval(session=self.sess).reshape(
                (self.args.seqLen, self.args.numMotifs));
            if self.args.trainStrandedActivities > 0:
                outFile.write("\tpositionalActivityBiasRC");
                positionalActivityBiasRCVals = self.positionalActivityBiasRC.eval(session=self.sess).reshape(
                    (self.args.seqLen, self.args.numMotifs));
            outFile.write("\n");
        ...
        if self.args.accIsAct > 0:
            outFile.write("-1\t%g\t%g" % (
            accActivity.eval(session=self.sess), self.constant.eval(session=self.sess)));  # intercepts
        else:
